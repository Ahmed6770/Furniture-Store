...
function OrderSummary({ subtotal = 0, shipping = 0, showCheckoutButton = true }) {
    ...
    <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-2xs sticky top-24">
      ...
      <div className="space-y-3 text-sm">
        <div className="flex items-center justify-between text-stone-600">
          <span>Subtotal</span>
          <span className="font-semibold text-stone-900">${isCartEmpty ? "0.00" : subtotal.toFixed(2)}</span>
        </div>
        ...
        <div className="flex items-center justify-between text-stone-600">
          ...
          <span className="font-semibold text-stone-900">${isCartEmpty ? "0.00" : shipping.toFixed(2)}</span>
        </div>
      </div>
      ...
      {showCheckoutButton && (
        <div className="space-y-3">
          {isCartEmpty ? (
            <button className="w-full cursor-not-allowed rounded-xl bg-stone-300 py-3.5 text-center text-sm font-bold text-stone-500 flex items-center justify-center gap-2"
              disabled
            >
              <Lock size={16} />
            ...
            <Link className="w-full cursor-pointer rounded-xl bg-[#b53d5d] py-3.5 text-center text-sm font-bold text-white shadow-sm transition hover:bg-[#962f49] flex items-center justify-center gap-2"
              to="/checkout"
            >
              <Lock size={16} />
              <span>Proceed to Checkout</span>
